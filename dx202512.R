...
wddat$panel<-as.numeric(wddat$panel)
...
for (i in 1:ngroup){
  ...
  for (j in 1:nsmpgrp){
    ...
    nevalsmp<-length(evalsmp)
    cdata<-data.frame(matrix(NA,nevalsmp+2,nfactor))
    cdata[1:2,]<-mm
    for (k in 1:nevalsmp){
    ...
    ttl<-paste0(groups[i],'-Sample ',smpgrp[j])
    #print(cdata)
    radarchart(cdata,
    ...
    for (k in 1:nevalsmp){
      ...
      gp<-ggplot(data=thispdata, aes(x=as.factor(fact),y=as.numeric(value)))+geom_boxplot()
      gp<-gp+geom_jitter(width=0.1, height=0)+ylim(c(-3,3))
